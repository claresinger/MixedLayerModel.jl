...
function run_mlm_ss(params, dt=3600.0*5.0, tspan=3600.0*24.0*10.0)
    ...
    @time begin
        ...
        sol = solve(prob, DynamicSS(Euler(); abstol=0.0, reltol=tol, tspan=tspan), dt=dt, progress=true, progress_steps=30);
...
"""
    ...
    and save output to file
"""
function run_mlm_ss_from_init(u0, params, dt=3600.0*5.0, tspan=3600.0*24.0*10.0, filename="default.txt")
    params.qft0 = calc_qft0(params.RHft, params.Gamma_q, params.sft0, params.Gamma_s);
