...
p_decoup = scatter(co2, De, color=c, marker=:x, markersize=ms, label="", 
...
plot!(p_decoup, co2, De, linewidth=2, linestyle=:dot, color=c, label="")
...
scatter!(p_decoup, co2, De, color=c, marker=:x, markersize=ms, label="")
...
dR, De = zeros(N), zeros(N);

for (i, co2i) in enumerate(co2u)
    local file = "experiments/output/"*exp_path*"co2_upstep_"*string(co2i)*".jld2"
    local dat = load(file);
    local uf = dat["uf"];
    local par = dat["p"];
    ...
    De[i] = calc_decoupling(uf, par, zbi, lwp[i]);
end

ms = 8
c = "crimson"
scatter!(p_dR, co2u, dR, color=c, marker=:circle, markersize=ms, markerstrokewidth=0, label="")
scatter!(p_decoup, co2u, De, color=c, marker=:circle, markersize=ms, markerstrokewidth=0, label="")
...
scatter!(p_zi, co2u, zb, color=c, marker=:star, markersize=ms, markerstrokewidth=0, label="")
...
plot!(p_decoup, co2u, De, color=c, linewidth=2, label="")
...
plot!(p_zi, co2u, zb, color=c, linewidth=2, label="")
...
for (i, co2i) in enumerate(co2d)
    local file = "experiments/output/"*exp_path*"co2_downstep_"*string(co2i)*".jld2"
    local dat = load(file);
    local uf = dat["uf"];
    local par = dat["p"];
    zii, sM, qM, ssti, cfi = uf;
...
scatter!(p_decoup, co2d, De, color=c, marker=:circle, markersize=ms, markerstrokewidth=0, label="")
...
scatter!(p_zi, co2d, zb, color=c, marker=:star, markersize=ms, markerstrokewidth=0, label="")
...
plot!(p_decoup, co2d, De, color=c, linewidth=2, label="")
...
plot!(p_zi, co2d, zb, color=c, linewidth=2, label="")
